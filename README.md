# Olá, eu sou a Camille! ✨​

<p align="center">
  <img src="https://img.shields.io/badge/Java-17+-e015a0?style=for-the-badge&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring_Boot_3-e015a0?style=for-the-badge&logo=spring&logoColor=white" />
  <img src="https://img.shields.io/badge/TypeScript-e015a0?style=for-the-badge&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/React-e015a0?style=for-the-badge&logo=react&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-e015a0?style=for-the-badge&logo=postgresql&logoColor=white" />
</p>

💡 Desenvolvedora Java focada em APIs REST e arquitetura limpa | Em constante evolução

### 💖 Sobre mim

> Desenvolvedora Full Stack Java Jr. com foco em back-end, apaixonada por construir APIs bem estruturadas, seguras e escaláveis com Spring Boot.  
> Gosto de transformar problemas em soluções práticas e estou sempre evoluindo (atualmente explorando microsserviços e Docker).  
> 🚀 Em busca da minha primeira oportunidade para crescer rápido, aprender muito e gerar impacto real com tecnologia.

---


### ⚕️ Habilidades Técnicas

> * **Back-end:** Spring Boot, Spring Security, JPA  
> * **Linguagens:** Java, TypeScript, JavaScript, SQL  
> * **Banco de Dados:** PostgreSQL, MySQL  
> * **Arquitetura:** REST APIs, JWT, MVC, Hexagonal  
> * **Testes:** JUnit, Mockito, SonarQube  

---

### 🥇 Projetos

> * **Quadro Kanban:** Sistema de gestão de tarefas com Arquitetura Hexagonal, garantindo alta manutenibilidade e desacoplamento.
> > 🔗 https://github.com/CahTarine/Kanban  
> * **Carona:** Plataforma completa com autenticação JWT, integração com APIs externas e regras de negócio complexas.
> > 🔗 https://github.com/CahTarine/React-Aplicativo-Carona

---

### 🔮 Formação 

> * 🎓 **ADS:** Universidade Pitágoras Unopar Anhanguera (Concluído em 2025).
> * 🚀 **Full Stack:** 447 horas de formação intensiva pela **Generation Brasil**.
> * 🇬🇧 **Idiomas:** Inglês (em aprendizado contínuo).

---

### ⚙️ GitHub Analytics

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=CahTarine&theme=radical&cache=1" alt="GitHub Streak" />
</p>

---

### 📫 Vamos nos conectar?

💼 Aberta a oportunidades como Desenvolvedora Java Jr.

<p align="left">
  <a href="https://linkedin.com/in/camille-tarine">
    <img src="https://img.shields.io/badge/LinkedIn-e015a0?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:devcamilletarine@gmail.com">
    <img src="https://img.shields.io/badge/Email-e015a0?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

